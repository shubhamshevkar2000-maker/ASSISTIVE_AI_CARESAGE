"""Quick reseed script — adds 10 realistic patients to the queue."""
...
if not dept:
    print("ERROR: No active department found. Run seed_data first!")
    sys.exit(1)

print(f"Seeding into department: {dept.name}")
...
for s in scenarios:
    try:
        ...
        result = compute_triage(str(enc.id), s['vitals'], s['syms'], {}, s['age'])
        print(f"  + {s['name']} ({s['age']}y) -> {result['priority'].upper()} score={result['effective_score']}")
        created += 1
    except Exception as e:
        print(f"  ERROR {s['name']}: {e}")

total = Encounter.objects.filter(is_deleted=False).count()
print(f"\nDone! Created {created}/10 patients. Total active encounters: {total}")
